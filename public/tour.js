...
(() => {
  ...
  const STEPS = [
    ...
    { t: "Due strade, un solo stato", d: "Asta guidata: Fantalgoritmo costruisce la tua rosa ideale, la aggiorna a ogni chiamata e ti dice se prendere il giocatore appena uscito. Assistente asta: scegli tu il giocatore e chiedi quanto spendere. Stesso budget e stessa rosa in entrambe.", target: "#so-choose .modes", prep: async () => { await soloMode(); $("so-setup").classList.add("hidden"); GUIDED.reset(); SOLO.setMode("choose"); window.scrollTo(0, 0); } },
    { t: "La tua rosa ideale", d: "Con 500 FM, 8 squadre e 3-8-8-6 Fantalgoritmo ottimizza la migliore rosa realmente acquistabile: budget per reparto, uno slot per ogni giocatore con fascia, prezzo target, limite massimo, alternative, e la priorità di acquisto. I prezzi attesi includono un premio di scarsità che cresce con il numero di squadre.", target: "#g-plan", prep: async () => { SOLO.setMode("guided"); await sleep(80); $("g-plan").scrollIntoView({ block: "start" }); }, skipScroll: true },
    { t: "Viene chiamato Lautaro", d: "In alto scrivi il nome del giocatore appena uscito. Fantalgoritmo risponde subito: quanto è coerente con la tua strategia, fino a quanto conviene spingere e perché. Qui il verdetto: non era nel piano e a 128 FM è troppo caro → LASCIALO.", target: ".gcalled", prep: async () => { GUIDED.setCalled("MARTINEZ|INT|ATT", "128"); SOLO.render(); window.scrollTo(0, 0); } },
    { t: "Preso da altri → la rosa si aggiorna", d: "Registri com'è finita con un tap. Lautaro esce dal mercato, il prezzo entra nell'inflazione e la rosa ideale sotto viene ricostruita: budget per reparto, target e strategia possono cambiare.", target: ".gbanner", prep: async () => { $("gc-other")?.click(); await sleep(160); window.scrollTo(0, 0); }, target2: "#g-plan" },
    { t: "Scarsità attacco ↑", d: "Anche Thuram va a un avversario a 103 FM. Fantalgoritmo misura che la qualità rimasta per coprire la domanda di attaccanti sta calando: segnala SCARSITÀ ATTACCO ↑, alza la priorità e, se conviene, cambia strategia.", target: ".gbanner", prep: async () => { $("gb-close")?.click(); await sleep(60); GUIDED.setCalled("THURAM|INT|ATT", "103"); SOLO.render(); await sleep(60); $("gc-other")?.click(); await sleep(160); window.scrollTo(0, 0); } },
    { t: "Viene chiamato il tuo obiettivo", d: "Questa volta è un giocatore della tua rosa ideale: coerenza 100%, PRENDILO, con il limite da non superare. Scrivi il prezzo e conferma \"l'ho preso io\".", target: ".gcalled", prep: async () => { $("gb-close")?.click(); await sleep(60); const P = GUIDED.getPlan(); const s = P.slots.slice().sort((a, b) => b.target - a.target)[0]; GUIDED.setCalled(s.p.id, String(Math.min(s.limit, s.target + 8))); SOLO.render(); window.scrollTo(0, 0); } },
    { t: "Entra nella rosa", d: "Il giocatore passa tra i presi con il prezzo pagato, il budget scende e il piano spiega dove recupera la differenza. Non genera una squadra una volta sola: segue tutta l'asta. Se sbagli un tap, ↩ Annulla ripristina tutto.", target: ".gbanner", prep: async () => { $("gc-mine")?.click(); await sleep(180); window.scrollTo(0, 0); }, target2: "#g-plan" },
    // --- assistente

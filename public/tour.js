...
(() => {
  ...
  const STEPS = [
    ...
    { t: "Due strade, un solo stato", d: "Asta guidata: Fantalgoritmo costruisce la tua rosa ideale, la aggiorna a ogni chiamata e ti dice se prendere il giocatore appena uscito. Assistente asta: scegli tu il giocatore e chiedi quanto spendere. Stesso budget e stessa rosa.", target: "#so-choose .modes", prep: async () => { await soloMode(); SOLO.setMode("choose"); GUIDED.reset(); window.scrollTo(0, 0); } },
    { t: "Strategia: scegli il tuo stile", d: "Prima dell'asta scegli la strategia: Fantalgoritmo (consigliata), Aggressiva, Convenzionale o Personalizzata. Ogni card mostra un'anteprima concreta calcolata sul tuo mercato: ripartizione del budget, profilo della rosa, rischio, punto di forza e criticità. La strategia cambia raramente; il piano ogni volta che serve.", target: ".gstrat.on", prep: async () => { SOLO.setMode("guided"); const S = SOLO.get(); delete S.strategy; SOLO.save(); GUIDED.reset(); SOLO.render(); await sleep(200); window.scrollTo(0, 0); }, skipScroll: true },
    { t: "Piano A: la tua rosa ideale", d: "Con 500 FM, 8 squadre e 3-8-8-6 Fantalgoritmo ottimizza la migliore rosa realmente acquistabile: budget per reparto, slot con priorità dinamica (P1 obiettivo principale, P2 alternativa, P3 value, P4 copertura, P5 emergenza), fascia, target, massimo e alternative. I prezzi includono il premio di scarsità legato ai partecipanti.", target: "#g-plan", prep: async () => { const b = document.querySelector("[data-pick=fa]"); if (b) { b.click(); await sleep(300); } GUIDED.openPlan(true); SOLO.render(); await sleep(60); $("g-plan-toggle").scrollIntoView({ block: "start" }); }, skipScroll: true },
    { t: "Dashboard e prossima mossa", d: "Sempre visibili: presi, residuo, coerenza strategica, qualità della rosa (0–100), rischio e fase dell'asta. Sotto, la prossima mossa consigliata: cosa cercare, budget massimo, cosa evitare. Si aggiorna a ogni evento.", target: ".gnextmove", prep: async () => { window.scrollTo(0, 0); }, target2: ".gstatus" },
    { t: "La tua strategia, sempre sotto controllo", d: "Il pannello traduce i numeri in messaggi: budget previsto / aggiornato / speso per reparto, obiettivi principali e stato (✓ in linea, ⚠ sopra il piano). Qui vedi se stai rispettando il piano.", target: ".gpanel", prep: async () => { document.querySelector(".gpanel").scrollIntoView({ block: "center" }); }, skipScroll: true },
    { t: "Tattica: viene chiamato Lautaro", d: "Scrivi il nome del giocatore appena uscito. Fantalgoritmo risponde con coerenza strategica, valore, fasce di prezzo (ideale, corretto, massimo strategico, spendibile ora), verdetto e motivazione. Un giocatore forte può essere sbagliato per la tua asta: viene detto esplicitamente.", target: ".gcalled", prep: async () => { window.scrollTo(0, 0); GUIDED.setCalled("MARTINEZ|INT|ATT", "128"); SOLO.render(); await sleep(120); window.scrollTo(0, 0); } },
    { t: "Perché? e Simula acquisto", d: "Ogni consiglio è spiegabile: quattro ragioni concrete. E prima di confermare puoi simulare: budget residuo, coerenza, budget del reparto, se il Piano A resta realizzabile e quali obiettivi restano acquistabili o diventano incompatibili.", target: ".gsim", prep: async () => { GUIDED.setFlags(true, true); SOLO.render(); await sleep(150); document.querySelector(".gsim")?.scrollIntoView({ block: "center" }); }, target2: ".gwhy", skipScroll: true },
    { t: "Preso da altri → il piano si aggiorna", d: "Registri com'è finita con un tap. Lautaro esce dal mercato, il prezzo entra nella lettura del tuo mercato e Fantalgoritmo passa allo scenario coerente con la strategia, senza rifare tutto da zero: budget per reparto, target e priorità.", target: ".gbanner", prep: async () => { GUIDED.setFlags(false, false); if ($("gc-other")) { $("gc-other").click(); await sleep(200); } window.scrollTo(0, 0); }, target2: ".gnextmove" },
    { t: "Scarsità e pressione competitiva", d: "Anche Thuram va a un avversario a 103 FM. Fantalgoritmo misura la qualità rimasta per la domanda del ruolo (SCARSITÀ ATTACCO ↑), la pressione competitiva per reparto e la fase dell'asta, e cambia priorità o piano se conviene.", target: ".gbanner", prep: async () => { $("gb-close")?.click(); await sleep(60); GUIDED.setCalled("THURAM|INT|ATT", "103"); SOLO.render(); await sleep(120); $("gc-other")?.click(); await sleep(200); window.scrollTo(0, 0); } },
    { t: "🔥 Occasione", d: "Viene chiamato Kean e il prezzo è fermo a 60, molto sotto il valore. Non era nel Piano A, ma l'indice di opportunità è alto: Fantalgoritmo segnala l'OCCASIONE e dice COMPRA fino al massimo strategico. Il piano è una mappa, non un vincolo.", target: ".gcalled", prep: async () => { $("gb-close")?.click(); await sleep(60); GUIDED.setCalled("KEAN|FIO|ATT", "60"); SOLO.render(); await sleep(150); window.scrollTo(0, 0); } },
    { t: "L'ho preso: entra nella rosa", d: "Confermi il prezzo: il giocatore passa tra i presi, il budget scende, coerenza e qualità si aggiornano e il piano ridistribuisce il resto. Se paghi oltre il limite, compare subito un piano di recupero. ↩ Annulla ripristina tutto.", target: ".gbanner", prep: async () => { if ($("gc-mine")) { $("gc-mine").click(); await sleep(250); } window.scrollTo(0, 0); }, target2: ".gstatus" },
    { t: "Piano B e Piano C", d: "Non una sola rosa rigida: il Piano B mostra come cambia la rosa se perdi i due obiettivi principali; il Piano C è lo scenario opportunistico. Sotto, il blocco portieri valutato nel suo insieme, il mercato della tua asta e i rischi di rosa.", target: ".gscen", prep: async () => { $("gb-close")?.click(); await sleep(60); GUIDED.setFlags(false, false, true); SOLO.render(); await sleep(200); document.querySelector(".gscen")?.scrollIntoView({ block: "center" }); }, skipScroll: true },
    // --- assistente
    { t: "Assistente asta: quando decidi tu", d: "Stessa rosa e stesso budget dell'Asta guidata (il giocatore appena preso è già qui). Scegli il giocatore e chiedi: quanto posso spendere?", target: "#so-tabs", prep: async () => { $("gb-close")?.click(); SOLO.setMode("assist"); await soloTab("asta"); window.scrollTo(0, 0); } },
